...
exports.signUp = async (req, res) => {
    try {
        if(req.body.confirmPassword !== req.body.password || !req.body.confirmPassword) {
            return res.status('400').json({error: RevendeurErrors.revendeurError.passwordAndConfirmPasswordIsMatch})
        }
...
exports.CheckIfEmailIsExist = async (req, res, next) => {
    try {
        const count = await RevendeurModel.countDocuments({email: req.body.email}).exec()

        if (count !== 0)
            return res.status(400).json({ error: RevendeurErrors.revendeurError.checkThisEmailIfAlreadyExist });
...
exports.signIn = async (req, res) => {
    try {
        const foundRevendeur = await RevendeurModel.findOne({email: req.body.email})
        if(!foundRevendeur){
            return res.status(404).json({error: RevendeurErrors.revendeurError.checkThisEmailIfNotExist})
        }

        const hashedPassword = await generateSaltedHash(req.body.password, foundRevendeur.password.salt, foundRevendeur.password.uuid)

        if(hashedPassword.hash !== foundRevendeur.password.hash){
            return res.status(401).json({error: RevendeurErrors.revendeurError.invalidUserPassword})
        }
        ...
        try {
                ...
                const accessToken = jwt.sign(
                    { _id: foundRevendeur._id, exp: Math.floor(Date.now() / 1000) + (3 * 60 * 60) },
                    cert,   
                    { algorithm: 'RS512' }
                );

                const refreshToken = jwt.sign(
                    { _id: foundRevendeur._id, exp: Math.floor(Date.now() / 1000) + (24 * 60 * 60) },
                    cert,   
                    { algorithm: 'RS512' }
                );
...
exports.getOneRevendeur = async (req, res) => {
    try {
        ...
        const revendeur = await RevendeurModel.findById(revendeurId)
